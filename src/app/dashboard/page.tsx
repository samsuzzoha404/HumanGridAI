...
const WalletView = dynamic(() => import('@/components/wallet/WalletView').then(mod => ({ default: mod.WalletView })), {
  ssr: false,
  loading: () => <div className="flex items-center justify-center h-96">Loading...</div>
});

const ProfileView = dynamic(() => import('@/components/profile/ProfileView').then(mod => ({ default: mod.ProfileView })), {
  ssr: false,
  loading: () => <div className="flex items-center justify-center h-96">Loading...</div>
});
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        setActivities(activitiesData.length > 0 ? activitiesData : mockActivityFeed);
      } catch (error) {
  ...
  const handleSkipTask = useCallback(async (taskId: string | number) => {
  ...
  const handleTaskTimeout = useCallback(async (taskId: string | number) => {
  ...
  const handleCompleteTask = useCallback(async (taskId: string | number, reward: number) => {
    try {
